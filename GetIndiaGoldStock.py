...
def getGoldPivots(High,Low,Close):
    Points={}
    #pivot,re1,re2,re3,re4,su1,su2,su3,su4,High,Close,Low
    #High=54.93
    #Low=53.09
...
def getLiveData():
    ...
    for TimeInterval in StrTimeIntervals:
        Candles = requests.get('https://kitecharts-aws.zerodha.com/api/chart/54261767/'+TimeInterval+'?public_token=Pb0MyvJ435vcAjx5fxCTtEV0oD0wdcwT&user_id=RM5678&api_key=kitefront&access_token=mwFtysT9JpZk6B7ZOUH4AqbrJkhUlTnC&from=2019-01-08&to=2019-02-05&ciqrandom=1549306657305') #GOLD
...
def Filter30MinUPCandles(FilteredCandles,iPatternName,CandleType,Size,quotes):        
    ...
    #print(FilteredCandles.iloc[ai]['Date'])
        ...
        LastCandleMinute=LastCandles.iloc[len(LastCandles)-1]['Date'].minute
        if(LastCandleMinute==0 | LastCandleMinute==30):
        #print(FilteredCandles.iloc[ai]['Date'])
...
def DisplayChart():
    ...
    LowerList=PriceList[21:-1]
    HigherList=PriceList[15:-7]
    ...
    plot(fig, filename="chart2-"+date1.strftime("%b-%d")+".html",auto_open=False)
...
if(5==5):
    ...
    S4=pivot+(Range*3)
    P["R4"]=R4
    P["R3a"]=MaRound((R3+R4)/2)
    P["R3"]=R3
    P["R2a"]=MaRound((R2+R3)/2)
    P["R2"]=R2
    P["R1a"]=MaRound((R2+R1)/2)
    P["R1"]=R1
    P["R0a"]=MaRound((pivot+R1)/2)
    P["Pivot"]=pivot
    P["S0a"]=MaRound((pivot+S1)/2)
    P["S1"]=S1
    P["S1a"]=MaRound((S2+S1)/2)
    P["S2"]=S2
    P["S2a"]=MaRound((S2+S3)/2)
    P["S3"]=S3
    P["S3a"]=MaRound((S4+S3)/2)
    P["S4"]=S4
